...
- `web/`：前端页面，基于 React + Vite，首页使用瀑布流展示，详情浮层独立于列表渲染，避免打开时触发整页卡片联动缩放。
...
- 插件通过 GitHub Issue 采集生成的数据会直接提交到仓库里的 `data/` 和 `images/`。
- 本地发版本前先执行 `git pull --rebase origin main`，先把远程最新采集数据同步下来。
- 提交代码时不要无脑执行 `git add .`，先确认没有误改或误删 `data/`、`images/`。
- 不要对 `main` 做 `force push`，否则可能把远程已有采集记录直接覆盖掉。
- 如果只是更新前端代码而没有改动 `data/items/*.json` 和 `images/`，正常同步后再 push，不会把原有采集数据冲掉。
